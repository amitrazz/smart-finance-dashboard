          ...
          {query.trim().length === 0 ? (
            <div className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  { name: "Dashboard", tab: "dashboard", icon: <PieChart className="w-4 h-4 text-indigo-400" /> },
                  { name: "Accounts & Cash", tab: "accounts", icon: <Landmark className="w-4 h-4 text-emerald-400" /> },
                  { name: "Transactions", tab: "transactions", icon: <ArrowUpRight className="w-4 h-4 text-blue-400" /> },
                  { name: "File Import Wizard", tab: "imports", icon: <FileText className="w-4 h-4 text-cyan-400" /> },
                  { name: "Investments Portfolio", tab: "investments", icon: <PieChart className="w-4 h-4 text-amber-400" /> },
                  { name: "Goals & Emergency Fund", tab: "goals", icon: <Target className="w-4 h-4 text-purple-400" /> },
                ].map((item) => (
                  <button
                    key={item.tab}
                    onClick={() => navigateTo(item.tab as NavTab)}
              ...
              {isLoading ? (
                ...
                <div className="space-y-1">
                  {results.map((res) => (
                    <button
                      key={res.id}
                      onClick={() => navigateTo(res.link.replace("/", "") as NavTab)}
