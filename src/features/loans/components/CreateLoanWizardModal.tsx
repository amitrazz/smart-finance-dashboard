...
import { CreateLoanInput, LoanType, LoanInterestType, FinancialInstitution } from "../../../types";
...
export const CreateLoanWizardModal: React.FC<CreateLoanWizardModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const payload: CreateLoanInput = {
      ...
      tenureMonths: tenure,
      startDate: startDt,
      // The backend's fixed-EMI field is `installmentAmount`, not `monthlyEmi` —
      // without this mapping the schedule silently falls back to a freshly
      // computed textbook EMI instead of the user's real bank EMI.
      installmentAmount: formData.monthlyEmi || undefined,
          ...
          {step === 1 && (
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                ...
                <div>
                  <label htmlFor="loan-institution" className="text-slate-400 font-semibold block mb-1.5">Lender Institution</label>
                  <InstitutionPicker
                    id="loan-institution"
