...
interface InsightsMobileNavDrawerProps {
  isOpen: boolean;
  onClose: () => void;
...
export const InsightsMobileNavDrawer: React.FC<InsightsMobileNavDrawerProps> = ({
  isOpen,
  onClose,
  sections,
  currentSubTab,
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="relative w-full max-w-lg bg-slate-900 border-t sm:border border-slate-800 rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-in slide-in-from-bottom duration-200">
        ...
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          <h3 className="text-sm font-bold text-slate-100">Financial Intelligence Navigation</h3>
          <button
            onClick={onClose}
        ...
        <div className="p-4 overflow-y-auto space-y-4">
          {sections.map((sec) => {
            const Icon = sec.icon;
            const isSecActive = sec.subTabs.some((s) => s.id === currentSubTab);
